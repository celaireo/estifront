...
export default function ListEmployes() {
    ...
    useEffect(() => {
        axios.get(`https://estiback.vercel.app/employes`).then((response) => {
    ...
    const getData = (e) => {
        axios.get(`https://estiback.vercel.app/employes?q=${e}`).then((getData) => {
                setAPIData(getData.data);
            });
    };

    const onDelete = (Id_employe) => {
        axios.delete(`https://estiback.vercel.app/employes/${Id_employe}`).then(() => {
            getData();
        })
    }
    ...
    const like = (data) => {
        data.like += 1;
        axios.put(`https://estiback.vercel.app/employes/${data.Id_employe}`, data)
    }
